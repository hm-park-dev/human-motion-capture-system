...
/* Private define ------------------------------------------------------------*/
/* extern node_securityParams_t NodeSecurityParams;
extern MOBLEUINT8 NetKey[16];
extern MOBLEUINT8 AppKey[16];
extern MOBLEUINT8 DevKey[16];
extern MOBLEUINT16 NetKeyIndex;
extern MOBLEUINT16 AppKeyIndex;
extern MOBLEUINT8 KeySize; */
...
#if (FSM_FF_ENABLE == 1)  // Free Fall Alert  Node
    if(FreeFall_Timer)
    {
      if((HAL_GetTick() - tickStart) > 3000)
      {  
        publishAddress = BluenrgMesh_GetAddress();
        // Send the command to turn Off the Free Fall Alert Cube light
        Generic_PublishOff(publishAddress);
        FreeFall_Timer = 0;
       }
    }
#endif
    
    /* Machine Learning Core or Finite State Machine */
    if(SendMCL_FSM_EMB)
    {  
      SendMCL_FSM_EMB = 0;
      lsm6dsox_all_sources_t status;
      lsm6dsox_all_sources_get(&dev_lsm6dsox, &status);

      // Check if interrupt is from Machine Learning Core
      if(status.mlc1) {
          uint8_t mlc_out[8];
          /* Read interrupt source registers in interrupt mode */
          lsm6dsox_mlc_out_get(&dev_lsm6dsox, mlc_out);
      
#if (MLC_6D_ENABLE == 1) // 6DOF Node
...
//          publishAddress = BluenrgMesh_GetPublishAddress(elementNumber, GENERIC_MODEL_SERVER_LEVEL_MODEL_ID);
          ...
          switch (mlc_out[0]) {
          case 1:
                  // Send the command to turn On the 6DOF Cube light
                  Generic_PublishOn(publishAddress);
                  BytesToWrite = sprintf((char *) BufferToWrite, "MLC: X-axis pointing up\r\n");
                  break;
          case 2:
                  //Send the command to turn On the 6DOF Cube light
                  Generic_PublishOn(publishAddress);
                  BytesToWrite = sprintf((char *) BufferToWrite, "MLC: X-axis pointing down\r\n");
                  break;
          case 3:
                  //Send the command to turn On the 6DOF Cube light
                  Generic_PublishOn(publishAddress);
                  BytesToWrite = sprintf((char *) BufferToWrite, "MLC: Y-axis pointing up\r\n");
                  break;
          case 4:
                  //Send the command to turn On the 6DOF Cube light
                  Generic_PublishOn(publishAddress);
                  BytesToWrite = sprintf((char *) BufferToWrite, "MLC: Y-axis pointing down\r\n");
                  break;
          case 5:
                  //Send the command to turn Off the 6DOF Cube light
                  Generic_PublishOff(publishAddress);
                  BytesToWrite = sprintf((char *) BufferToWrite, "MLC: Z-axis pointing up\r\n");
                  break;
          case 6:
                  // Send the command to turn On the 6DOF Cube light
                  Generic_PublishOn(publishAddress);
                  BytesToWrite = sprintf((char *) BufferToWrite, "MLC: Z-axis pointing down\r\n");
                  break;
          default:
                  break;
          }

#elif MLC_FANMON_ENABLE == 1 // Smart Vibration Sensor Node
//          elementNumber = Bluenrg_GetElementNumber();
//          publishAddress = BluenrgMesh_GetPublishAddress(elementNumber, GENERIC_MODEL_SERVER_LEVEL_MODEL_ID);
          publishAddress = BluenrgMesh_GetAddress();
          ...
          if(mlc_out[0] == 0)
          {
              ...
              {
                vibration_state = 1;
                BytesToWrite = sprintf((char *) BufferToWrite, "MLC: Low Vibration\r\n");
                Enable_SPI_IRQ();
                // Turn OFF the Vibration Cube light
                Generic_PublishOff(publishAddress);
              }
          }
          else{
              ...
              {
                vibration_state = 2;
                BytesToWrite = sprintf((char *) BufferToWrite, "MLC: High Vibration\r\n");
                Enable_SPI_IRQ();
                //Turn ON the Vibration Cube light
                Generic_PublishOn(publishAddress);
              }
          }
#endif
	} // End check if interrupt is from Machine Learning Core

	// Check if interrupt is from Finite State Machine
	if(status.free_fall)
        {
#if (FSM_FF_ENABLE == 1)  //Free Fall Alert  Node
//	      events[0].event_exist = 1;
//	      events[0].event_timestamp = (uint16_t) (HAL_GetTick() >> 3);
//	      events[0].event_type = FSM_DATA_FF;
//	      events[0].event_code = 0;
            BytesToWrite = sprintf((char *) BufferToWrite, "FSM: Free Fall\r\n");
            FreeFall_Timer = 1;
...
//            publishAddress = BluenrgMesh_GetPublishAddress(elementNumber, GENERIC_MODEL_SERVER_LEVEL_MODEL_ID);
            publishAddress = BluenrgMesh_GetAddress();
            Enable_SPI_IRQ();
            //Send the command to turn On the Free Fall Alert Cube light
            Generic_PublishOn(publishAddress);
#endif
	}
	// End check if interrupt is from Finite State Machine
    }
...
#ifdef STM32L4R9xx
  /************************STM32L4R9xx 96 bit Unique ID addresses***************/
#define DEVICE_ID_REG0 (0x1FFF7A10) 
#define DEVICE_ID_REG1 (0x1FFF7A14) 
#define DEVICE_ID_REG2 (0x1FFF7A18) 
#endif
